...
const storage = {
    async getParkingLot() {
        ...
        return (parkingLot ?? { parkingLotGroupId: -1, groups: [] });
    },
    async setParkingLot(parkingLot) {
...
async function buildTabsTree() {
    ...
    for (let i = 0; i < allTabs.length; i++) {
        ...
        if (!tabGroup || subGroupIds.has(tabGroup.id))
            continue;
        // Check if already processed as parent
        if (result.some((g) => g.groupId === tabGroup.id))
            continue;
...
async function collapseParentGroup(tabGroupId) {
    const tabGroups = await storage.getTabTree();
    const tabGroupData = tabGroups.find((g) => g.groupId === tabGroupId);
    if (!tabGroupData?.subGroups?.length)
        return;
    const subGroupTabIds = tabGroupData.subGroups.flatMap((sg) => sg.tabs.map((t) => t.id));
    if (!subGroupTabIds.length)
        return;
...
async function expandParentGroup(tabGroupId) {
    const parkingLot = await storage.getParkingLot();
...
chrome.tabGroups.onUpdated.addListener(async (tabGroup) => {
    ...
    if (found.group.collapsed === tabGroup.collapsed)
        return;
